...
def test_api_organization_a_cannot_retrieve_organization_b_source(client, db_session):
    ...
    # Not reachable without asserting the right organization (and without
    # any organization_id at all, this now also requires authentication
    # for a GLOBAL-only lookup, which this org-scoped source isn't)...
    assert (
        client.get(f"/api/v1/knowledge/sources/{source_a['id']}", headers=dev_auth_headers(member_a.id)).status_code
        == 404
    )
    # ...and not reachable by asserting the wrong one either. A valid id
    # belonging to a different organization must not leak the row, even
    # to an authenticated member of that other organization.
    response = client.get(
...
def test_api_organization_source_list_never_crosses_tenants(client, db_session):
    org_a = create_org(client, "Org A")
    org_b = create_org(client, "Org B")
    member_a = make_org_member(db_session, uuid.UUID(org_a["id"]))
    member_b = make_org_member(db_session, uuid.UUID(org_b["id"]))
    create_org_source(client, org_a["id"], member_a, name="A Source 1")
    create_org_source(client, org_a["id"], member_a, name="A Source 2")
    create_org_source(client, org_b["id"], member_b, name="B Source")

    org_a_sources = client.get(
        "/api/v1/knowledge/sources", params={"organization_id": org_a["id"]}, headers=dev_auth_headers(member_a.id)
    ).json()
    org_b_sources = client.get(
        "/api/v1/knowledge/sources", params={"organization_id": org_b["id"]}, headers=dev_auth_headers(member_b.id)
    ).json()
...
def test_api_global_knowledge_reachable_independent_of_any_tenant(client, db_session):
    ...
    # No organization exists for this reader at all — global knowledge
    # does not depend on tenant context to be retrievable, only on
    # *authentication* (item 9's own preserved rule).
    other_org = create_org(client, "Some Other Org")
    reader = make_org_member(db_session, uuid.UUID(other_org["id"]))
    response = client.get(f"/api/v1/knowledge/sources/{global_source['id']}", headers=dev_auth_headers(reader.id))
    assert response.status_code == 200
    assert response.json()["organization_id"] is None

    listing = client.get("/api/v1/knowledge/sources", headers=dev_auth_headers(reader.id)).json()
